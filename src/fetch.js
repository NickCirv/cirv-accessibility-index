...
// what a real browser (and the assistive tech layered on it) actually receives.
// A bot-view (the server's response to a non-browser agent) is not what users
// get, so it would understate real-world accessibility behaviour. We still play
// fair: we respect robots.txt (see robots.js), fetch only public homepages, send
// no cookies/credentials, and never execute JS or solve challenges.
